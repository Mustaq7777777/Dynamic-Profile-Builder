...
if ($id && isset($_SESSION['user_id'])) {
  ...
  $sqlDelete = "DELETE FROM workshop WHERE id = $id AND user_id = " . $_SESSION['user_id'];

  if (mysqli_query($conn, $sqlDelete)) {
    // Delete successful, set session variable and redirect
    $_SESSION["delete"] = "Workshop deleted successfully!";
    header("Location: windex.php"); // Redirect to workshop page (replace if needed)
  } else {
   ...
   if (mysqli_affected_rows($conn) == 0) {
    ...
    die("Error deleting workshops: " . mysqli_error($conn)); // Display other errors
